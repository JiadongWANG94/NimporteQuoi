...
#define TEST_BODY(test_case)                                           \
    ...
    LINFO(test_case) << "allocate 1 : " << p_1 << std::endl;           \
    void* p_1_2 = mp.Allocate(1);                                      \
    LINFO(test_case) << "allocate 1 : " << p_1_2 << std::endl;         \
    void* p_1_3 = mp.Allocate(1);                                      \
    LINFO(test_case) << "allocate 1 : " << p_1_3 << std::endl;         \
    void* p_2 = mp.Allocate(2);                                        \
    LINFO(test_case) << "allocate 2 : " << p_2 << std::endl;           \
    mp.Deallocate(p_2, 2);                                             \
    LINFO(test_case) << "deallocate." << std::endl;                    \
    void* p_2_2 = mp.Allocate(2);                                      \
    LINFO(test_case) << "allocate 2 : " << p_2_2 << std::endl;         \
    mp.Deallocate(p_2_2, 2);                                           \
    LINFO(test_case) << "deallocate." << std::endl;                    \
    void* p_2_3 = mp.Allocate(2);                                      \
    LINFO(test_case) << "allocate 2 : " << p_2_3 << std::endl;         \
    void* p_100 = mp.Allocate(100);                                    \
    LINFO(test_case) << "allocate 100 : " << p_100 << std::endl;       \
    void* p_300 = mp.Allocate(300);                                    \
    LINFO(test_case) << "allocate 300 : " << p_300 << std::endl;       \
    void* p_300000 = mp.Allocate(300000);                              \
    LINFO(test_case) << "allocate 300000 : " << p_300000 << std::endl; \
    int* p_int = mp.Construct<int>();                                  \
    LINFO(test_case) << "allocate int : " << p_int << std::endl;       \
    mp.Destroy(p_int);                                                 \
    mp.Deallocate(p_1, 1);                                             \
    for (int i = 1; i < 100; ++i) {                                    \
...
REGISTER_TEST(test_cmath) {
    LINFO(cmath) << "std::log2(1) = " << std::log2(1) << std::endl;
    LINFO(cmath) << "std::log2(2) = " << std::log2(2) << std::endl;
    LINFO(cmath) << "std::log2(3) = " << std::log2(3) << std::endl;
    LINFO(cmath) << "std::log2(8) = " << std::log2(8) << std::endl;
