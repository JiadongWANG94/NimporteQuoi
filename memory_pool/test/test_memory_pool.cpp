...
#define TEST_BODY(test_case)                                         \
    ...
    LOG(test_case) << "allocate 1 : " << p_1 << std::endl;           \
    void* p_1_2 = mp.Allocate(1);                                    \
    LOG(test_case) << "allocate 1 : " << p_1_2 << std::endl;         \
    void* p_1_3 = mp.Allocate(1);                                    \
    LOG(test_case) << "allocate 1 : " << p_1_3 << std::endl;         \
    void* p_2 = mp.Allocate(2);                                      \
    LOG(test_case) << "allocate 2 : " << p_2 << std::endl;           \
    mp.Deallocate(p_2, 2);                                           \
    LOG(test_case) << "deallocate." << std::endl;                    \
    void* p_2_2 = mp.Allocate(2);                                    \
    LOG(test_case) << "allocate 2 : " << p_2_2 << std::endl;         \
    mp.Deallocate(p_2_2, 2);                                         \
    LOG(test_case) << "deallocate." << std::endl;                    \
    void* p_2_3 = mp.Allocate(2);                                    \
    LOG(test_case) << "allocate 2 : " << p_2_3 << std::endl;         \
    void* p_100 = mp.Allocate(100);                                  \
    LOG(test_case) << "allocate 100 : " << p_100 << std::endl;       \
    void* p_300 = mp.Allocate(300);                                  \
    LOG(test_case) << "allocate 300 : " << p_300 << std::endl;       \
    void* p_300000 = mp.Allocate(300000);                            \
    LOG(test_case) << "allocate 300000 : " << p_300000 << std::endl; \
    int* p_int = mp.Construct<int>();                                \
    LOG(test_case) << "allocate int : " << p_int << std::endl;       \
    mp.Destroy(p_int);                                               \
    mp.Deallocate(p_1, 1);                                           \
    for (int i = 1; i < 100; ++i) {                                  \
